...
async function getProjectPermissionsList(projectId: string | number) {
	let permissions: Permission[] = [];
	try {
		permissions = await getProjectPermissionById(projectId);
	} catch (error) {
		permissions = [];
	}
...
async function getProjectApiKey(projectId: string | number) {
	...
	try {
		apiKey = await getProjectApiKeyById(projectId);
	} catch (error) {
		apiKey = undefined;
	}
			...
			try {
				...
				this.projects = await Promise.all(
					projects.map(async (project) => {
						const permissions: Permission[] = await getProjectPermissionsList(project.id);
