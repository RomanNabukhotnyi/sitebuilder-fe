...
export interface IUpdateSlotOrderParams {
	pageId: number,
	projectId: number,
	orders: Order[]
}
...
export const useSlotsStore = defineStore('slots', {
	...
	actions: {
		...
		async updateSlotOrder({ pageId, projectId, orders }: IUpdateSlotOrderParams): Promise<void> {
			try {
				const preparedOrders = orders.map((slot, index) => ({
					id: slot.id,
					order: index + 1,
				}));
				await updateSlotOrder(projectId, pageId, preparedOrders);
			} catch (error) {
				throw new IError(error);
			}
		},
